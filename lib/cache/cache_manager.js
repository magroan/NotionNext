...
/**
 * Upstream が参照する API:
 * キャッシュにあれば返し、無ければ getter() を実行して保存して返す。
 *
 * @param {string|any} key
 * @param {Function} getter async () => data
 * @param {boolean} forceCacheRead ENABLE_CACHE=falseでも読む
 * @returns {*}
 */
export async function getOrSetDataWithCache(key, getter, forceCacheRead = false) {
  const k = normalizeKey(key)
  ...
  // null/undefined はキャッシュしない（挙動を安全側に）
