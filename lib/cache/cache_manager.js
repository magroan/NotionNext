...
// Accept boolean-like values from env/Notion config (true/false/True/False/1/0/yes/no)
const toBool = v => {
...
export async function getDataFromCache(key, force) {
  if (toBool(BLOG.ENABLE_CACHE) || force) {
...
export async function setDataToCache(key, data, customCacheTime) {
  if (!enableCacheInVercel || !data || !toBool(BLOG.ENABLE_CACHE)) {
    return
  }
