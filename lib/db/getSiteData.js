...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  // 日付順にソート（DB の date プロパティを最優先）
  allPages.sort((a, b) => {
    const aDate = a?.date?.start_date || a?.publishDate
    const bDate = b?.date?.start_date || b?.publishDate
  ...
  // "注目" カテゴリの最新記事を 1 件だけ TOP に固定
  const FEATURED_CATEGORY = '注目'
  const featuredIndex = allPages.findIndex(
    p =>
      p?.type === 'Post' &&
      p?.status === 'Published' &&
      p?.category === FEATURED_CATEGORY
  )
  if (featuredIndex > 0) {
    const featuredPost = allPages[featuredIndex]
    allPages.splice(featuredIndex, 1)
    allPages.unshift(featuredPost)
  }
