...
function getLatestPosts({ allPages, from, latestPostCount }) {
  ...
  const latestPosts = Object.create(allPosts).sort((a, b) => {
    const rawA = a?.lastEditedDate || a?.publishDate
    const rawB = b?.lastEditedDate || b?.publishDate
    ...
    if (!dateA || isNaN(dateA.getTime())) {
      console.error(
        '[getLatestPosts] Invalid date for post A:',
        a?.title,
        'rawA=',
        rawA
      )
    ...
    if (!dateB || isNaN(dateB.getTime())) {
      console.error(
        '[getLatestPosts] Invalid date for post B:',
        b?.title,
        'rawB=',
        rawB
      )
