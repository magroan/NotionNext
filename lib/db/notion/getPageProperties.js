...
/**
 * Convert date-like value to YYYY-MM-DD (UTC).
...
export default async function getPageProperties(
  ...
  const publishDateLike = properties?.date?.start_date || value?.created_time
  properties.publishDate = publishDateLike ? new Date(publishDateLike).getTime() : undefined
  properties.publishDay = toYMD(publishDateLike)
  properties.lastEditedDate = value?.last_edited_time
  properties.lastEditedDay = toYMD(value?.last_edited_time)
