...
export default async function getPageProperties(
  ...
  // publishDay は「表示用」（ローカライズされる）なので Date パース用途では使わない
...
/**
 * publishDay（ローカライズ文字列）を Date パースに使うと壊れるので、
 * publishDate（timestamp）を優先して %year/%month/%day を作る。
 */
function generateCustomizeSlug(postProperties, NOTION_CONFIG) {
  ...
  allSlugPatterns.forEach((pattern, idx) => {
    if (pattern === '%year%') {
      fullPrefix += baseDate.getUTCFullYear()
    } else if (pattern === '%month%') {
      fullPrefix += String(baseDate.getUTCMonth() + 1).padStart(2, 0)
    } else if (pattern === '%day%') {
      fullPrefix += String(baseDate.getUTCDate()).padStart(2, 0)
  ...
  if (fullPrefix.startsWith('/')) fullPrefix = fullPrefix.substring(1)
  if (fullPrefix.endsWith('/')) fullPrefix = fullPrefix.substring(0, fullPrefix.length - 1)
