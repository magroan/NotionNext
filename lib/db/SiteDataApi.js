...
export async function resolvePostProps({
  ...
  let post = props?.allPages?.find(p => {
    ...
    const normalizedPageId = pageId ? idToUuid(pageId) : null
    const normalizedFullSlug =
      typeof fullSlug === 'string' && fullSlug.length >= 32 ? idToUuid(fullSlug) : null

    return (
      slugCandidates.has(p.slug) ||
      slugCandidates.has(p.slug?.split('/').pop()) ||
      (pageId && slugCandidates.has(pageId)) ||
      (normalizedPageId && slugCandidates.has(normalizedPageId)) ||
      (normalizedFullSlug && slugCandidates.has(normalizedFullSlug))
    )
  })
...
async function convertNotionToSiteData(
  ...
  if (siteConfig('POSTS_SORT_BY', null, NOTION_CONFIG) === 'date') {
    allPages.sort((a, b) => {
      const aFeatured = a?.category === '注目' ? 1 : 0
      const bFeatured = b?.category === '注目' ? 1 : 0
