...
export const convertVal = val => {
  ...
  // 兼容 Notion/手工输入的 True/False/None/null/undefined
  ...
  // 检测是否数字（整数/小数）并避免数值溢出
  if (/^-?\d+(\.\d+)?$/.test(val)) {
    const parsedNum = Number(val)
    // 如果是整数且大于 JS 最大安全整数，则保留为字符串
    if (Number.isInteger(parsedNum) && Math.abs(parsedNum) > Number.MAX_SAFE_INTEGER) {
      return val + ''
    }
    // NaN 兜底
  ...
  try {
    ...
    // 兼容 JSON 内部出现 True/False/None（Notion 表里很常见）
    val = val
