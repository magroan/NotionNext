...
export const convertVal = val => {
  ...
  // 统一处理前后空白，避免 " False " 这类污染导致无法识别
  const raw = val
  const trimmed = raw.trim()

  // 检测是否数字并避免数值溢出（允许前后空白）
  if (/^\d+$/.test(trimmed)) {
  ...
  // 检测是否为布尔值（兼容 True/False/ TRUE/FALSE 等）
  {
    const boolLike = trimmed.toLowerCase()
    if (boolLike === 'true' || boolLike === 'false') {
      return JSON.parse(boolLike)
    }
  }

  // 检测是否为 URL（保留原始值，避免破坏 URL 中的空白/编码语义）
  if (isUrlLikePath(trimmed)) {
    return trimmed
  }

  // 配置值前可能有污染的空格
  // 如果字符串中没有 '[' 或 '{'，则直接返回（返回原始字符串以尽量保持兼容）
  ...
  // 转换 [] , {} 这类字符串为对象
  try {
    const normalized = cleanJsonString(trimmed)
